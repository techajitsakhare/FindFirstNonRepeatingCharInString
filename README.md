...
#Complexity Analysis 
Complexity: O(N). You loop through the string twice, where N is the string length. This is already optimal for time.Space Complexity: O(U). You store unique characters in a dictionary. In the worst case, this requires O(N) space if all characters are unique.
